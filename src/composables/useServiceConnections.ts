...
export function useServiceConnections() {
     ...
     */
    const checkConnection = async (service: ServiceName, force: boolean = false): Promise<boolean> => {
    ...
    const checkAllConnections = async (force: boolean = false): Promise<ServiceConnectionStatus> => {
        ...
        const results = await Promise.all(
            SERVICE_NAMES.map(service => checkConnection(service, force))
        )
        ...
        connectionState.value = {
            status: connectedCount === 0 ? 'disconnected' : 
                    connectedCount === SERVICE_NAMES.length ? 'connected' : 'partial',
