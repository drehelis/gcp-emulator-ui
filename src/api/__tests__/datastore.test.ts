...
describe('datastoreApi', () => {
  ...
  describe('listKinds', () => {
    it('fetches kinds and caches result', async () => {
      ...
      mocks.mockPost.mockImplementation((url: string, data: any) => {
        ...
        if (url.includes(':runQuery') && (data.query?.kind?.[0]?.name === 'KindA' || data.query?.kind?.[0]?.name === 'KindB')) {
             return Promise.resolve({
                data: {
                    batch: {
                        entityResults: [
                            { entity: { key: { partitionId: { databaseId: '' }, path: [{ kind: 'KindA', id: '1' }] } } }
                        ]
                    }
                }
            })
  ...
  describe('createEntity', () => {
      it('commits entity and clears cache', async () => {
          ...
          const entity = { key: { partitionId: { projectId }, path: [{ kind: 'User', name: 'u1' }] }, properties: {} }
  ...
  describe('deleteEntity', () => {
      it('deletes entity', async () => {
          mocks.mockPost.mockResolvedValue({ data: { mutationResults: [{}] } })
          const key = { partitionId: { projectId, databaseId: '' }, path: [{ kind: 'User', name: 'u1' }] }
               ...
               if (url.includes(':runQuery')) {
                   ...
                   return Promise.resolve({
                       data: {
                           batch: {
                               entityResults: [
                                   { entity: { key: { path: [{ kind: 'Kind1', name: 'e1' }] } } }
                               ]
                           }
                       }
                   })
