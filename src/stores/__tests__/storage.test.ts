...
describe('useStorageStore', () => {
  ...
  describe('deleteObjects', () => {
    ...
    it('deletes folder contents recursively', async () => {
      ...
      vi.mocked(storageApi.listObjects)
        .mockResolvedValueOnce({ // Initial list (what is in the store)
           items: [],
           prefixes: ['folder1/'] 
        })
        .mockResolvedValueOnce({ // content of 'folder1/' - used by deletion logic
           items: [ 
      ...
      expect(storageApi.listObjects).toHaveBeenLastCalledWith(expect.objectContaining({
        bucket: 'my-bucket',
        prefix: 'folder1/'
      }))

      // Verify deleteMultipleObjects was called with internal files
      expect(storageApi.deleteMultipleObjects).toHaveBeenCalledWith('my-bucket', expect.arrayContaining([
        'folder1/fileA.txt',
