...
describe('useTopicsStore', () => {
  ...
  describe('getters', () => {
    ...
    it('filteredTopics applies date range filter', () => {
        ...
        store.filters.dateRange = { start: new Date(now.getTime() - 1000), end: new Date(now.getTime() + 1000) }
        expect(store.filteredTopics).toHaveLength(1)
  ...
  describe('createTopic', () => {
    ...
    it('throws if topic exists', async () => {
      ...
      await expect(store.createTopic({
        projectId: 'test-project',
        name: 'existing'
  ...
  describe('deleteTopic', () => {
    it('deletes topic and related subscriptions', async () => {
      ...
      vi.mocked(subscriptionsApi.getSubscriptions).mockResolvedValue([
        { name: 'projects/test-project/subscriptions/sub1', topic: 'projects/test-project/topics/topic1' },
        { name: 'projects/test-project/subscriptions/sub2', topic: 'projects/test-project/topics/other-topic' }
      ] as any)
  ...
  describe('batch operations', () => {
    it('createBatchTopics runs and completes', async () => {
      const store = useTopicsStore()
      const batchId = await store.createBatchTopics([
        { projectId: 'test-project', name: 'topic1' },
      ])
  ...
  describe('updateTopic errors', () => {
    it('throws if topic not found', async () => {
      const store = useTopicsStore()
      await expect(store.updateTopic('missing', {}, 'test-project')).rejects.toThrow('Topic missing not found')
    })
  })
