...
const apiMock = vi.hoisted(() => ({
    ...
    getKeyId: vi.fn((key: any) => key.path[key.path.length-1].name || key.path[key.path.length-1].id)
}))
...
describe('useDatastoreStore', () => {
  ...
  describe('loadKinds', () => {
      it('loads kinds and counts', async () => {
          ...
          vi.mocked(datastoreApi.getEntitiesByKind).mockResolvedValue({ entities: [], hasMore: false } as any)
