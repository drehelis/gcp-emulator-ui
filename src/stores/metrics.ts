...
export const useMetricsStore = defineStore(
  ...
  () => {
    ...
    const getSubscriptionMetrics = computed(
      () =>
        (subscriptionName: string): SubscriptionMetrics | null => {
          return subscriptionMetrics.value.get(subscriptionName) || null
        }
    )

    const getTotalMessagesPublished = computed(() => {
      return Array.from(topicMetrics.value.values()).reduce(
        (total, metrics) => total + metrics.messagesPublished,
        0
      )
    })
    ...
    function getTopicPublishRate(topicName: string, windowMinutes: number = 60): number {
      ...
      const recentPublishes = metrics.publishHistory.filter(entry => entry.timestamp >= windowStart)

      const totalMessages = recentPublishes.reduce((sum, entry) => sum + entry.messageCount, 0)
        ...
        if (data.topics) {
          data.topics.forEach(([key, value]: [string, any]) => {
            topicMetrics.value.set(key, {
              ...
              publishHistory:
                value.publishHistory?.map((h: any) => ({
                  ...h,
