...
export const useAppStore = defineStore('app', () => {
  ...
  const currentToasts = computed(() =>
    toasts.value.filter(toast => !toast.persistent || Date.now() - toast.timestamp.getTime() < (toast.duration || notifications.value.toastDuration))
  )
  ...
  function showToast(notification: Omit<ToastNotification, 'id' | 'timestamp'>) {
    ...
    const options = {
      timeout: notification.persistent ? false : (notification.duration || notifications.value.toastDuration)
    }
    ...
    if (notifications.value.enableDesktopNotifications && 'Notification' in window && Notification.permission === 'granted') {
  ...
  persist: {
    ...
    afterRestore: (context) => {
      ...
      if (context.store.theme && typeof document !== 'undefined') {
        const root = document.documentElement
        const shouldBeDark = context.store.theme === 'dark' ||
          (context.store.theme === 'auto' && window.matchMedia('(prefers-color-scheme: dark)').matches)
