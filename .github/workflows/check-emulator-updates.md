...
5. Before creating an issue, you MUST check if it already exists. Use your github tool to search for issues with: `includes emulator release notes in:title status:all repo:${{ github.repository }}`.
6. Review the resulting list of issues. If ANY of those issues are for this specific announcement (read the issue bodies if necessary by getting the issue details), DO NOT create a new issue.
